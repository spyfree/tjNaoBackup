...
#include <iostream>
#include <fstream>
PreviewController::PreviewController():lastz(-1)
{
	loadGainFile();
...
// 	P.c[2]  = Vector3<double>(0.0172, 0.0100, 0.0013);
	...
	K = Vector3<double>(702.6333,  250.3011,   27.7715);
	double tmp[MAX_PREVIEW_FRAMES] = {-235.584525349694, -88.7701867741906, -2.12740206986800, 46.6333453551716, 71.8219869018097, 82.5892052600125, 84.7549254387420, 81.9930692743559, 76.5986056551760, 69.9836482668832, 62.9978675153864, 56.1350503019956, 49.6659324061038, 43.7233333399018, 38.3564732884936, 33.5654124457076, 29.3226987412992, 25.5868093854165, 22.3103499487007, 19.4449234686106, 16.9439010770565, 14.7638848615344, 12.8653686895220, 11.2129187490780, 9.77507702467833, 8.52411476122261, 7.43571420324420, 6.48862582603772, 5.66432861201593, 4.94670859702506, 4.32176328338195, 3.77733488723134, 3.30287265991680, 2.88922299023203, 2.52844520371457, 2.21365063725500, 1.93886249850031, 1.69889410408715, 1.48924325653883, 1.30600072249204, 1.14577098831738, 1.00560367790084, 0.882934213093539, 0.775532476160970, 0.681458394109361, 0.599023507190179, 0.526757709171858, 0.463380456523138, 0.407775839038516, 0.358970987227914, 0.316117363491844, 0.278474546097100, 0.245396168509455, 0.216317722845801, 0.190745976063109, 0.168249781867938, 0.148452100959128, 0.131023067759769, 0.115673963816948, 0.102151977036698, 0.0902356422976367, 0.0797308731125610, 0.0704675061949405, 0.0622962913056017, 0.0550862678353224, 0.0487224774204885, 0.0431039686627911, 0.0381420558774330, 0.0337587988543666, 0.0298856749928084, 0.0264624189546314, 0.0234360082581418, 0.0207597760698444, 0.0183926349081282, 0.0162983971010504, 0.0144451796851441, 0.0128048830318476, 0.0113527338758768, 0.0100668846242408, 0.00892806187029203};
	//2
...
PrviewControllerOutput PreviewController::getRefCOM(PreviewBuffer& preBuffer, const PreviewWalkParam& walkParam)
{
	PrviewControllerState pcState;
	pcState.p.x = VecX[0]; pcState.v.x = VecX[1]; pcState.a.x = VecX[2];
	pcState.p.y = VecY[0]; pcState.v.y = VecY[1]; pcState.a.y = VecY[2];
	return getRefCOM(preBuffer, walkParam, pcState);
}
PrviewControllerOutput PreviewController::getRefCOM(PreviewBuffer& preBuffer, const PreviewWalkParam& walkParam, const PrviewControllerState& pcState)
{
	PrviewControllerOutput output;
	double uxk= 0, uyk = 0;
	double sumx1  = 0,sumx2 = 0,  sumy1 = 0, sumy2 = 0;
	int i = 0, j = 0;
	const PreviewState& stateV = preBuffer.front();
	Vector3<double> vecX(pcState.p.x, pcState.v.x, pcState.a.x);
	Vector3<double> vecY(pcState.p.y, pcState.v.y, pcState.a.y);
	...
	if(lastz == -1)
		lastz = preBuffer[0].COMz;
	double comz = stateV.COMz;
	double vcomz = (preBuffer[1].COMz - comz)/gainTable.dt;
	double acomz = (vcomz -  (comz - lastz)/gainTable.dt) / gainTable.dt;
	output.p.x = vecX[0];output.p.y = vecY[0]; output.p.z = comz;
	output.v.x = vecX[1]; output.v.y = vecY[1]; output.v.z = vcomz;
	output.a.x = vecX[2]; output.a.y = vecY[2]; output.a.z = acomz;
	
	return output;
}
// Vector3<double> PreviewController::getRefCOM(PreviewBuffer& preBuffer, const PreviewWalkParam& walkParam, Vector3<double>& mRBody, Vector3<double>& refRBody)
// {
// 	RotationMatrix Rbm =  RotationMatrix::getRotationX(mRBody.x).rotateY(mRBody.y).rotateZ(mRBody.z);
// 	RotationMatrix Rbref = RotationMatrix::getRotationX(refRBody.x).rotateY(refRBody.y).rotateZ(refRBody.z);
// 	Vector3<double> p = getRefCOM(preBuffer,walkParam);
// 	Vector3<double> pNew = Rbref.invert() * Rbm * p;
// 	return p + (pNew - p) * 0.05;
// }
