...
void PreviewWalkParam::setDefaultParams()
{
	...
	pRHipRollCompensatorRight = deg2rad(2.5);
	pLHipRollCompensatorRight = deg2rad(-2.5);
	pRHipRollCompensatorLeft = deg2rad(2.5);
	pLHipRollCompensatorLeft = deg2rad(-2.5);
	useHipRollCompensation = DISABLE;
	useHipRollCompensationRight = DISABLE;
	useHipRollCompensationLeft = DISABLE;
...
bool PreviewWalkParam::load(const std::string& filename)
{
	...
	if(fin){
		...
		if(readOK){
			...
			useHipRollCompensationRight = static_cast<int>(dbuf[10+numStages+64]);
			pRHipRollCompensatorRight = deg2rad(dbuf[10+numStages+65]);
			pLHipRollCompensatorRight = deg2rad(dbuf[10+numStages+66]);
			useHipRollCompensationLeft = static_cast<int>(dbuf[10+numStages+67]);
			pRHipRollCompensatorLeft = deg2rad(dbuf[10+numStages+68]);
			pLHipRollCompensatorLeft = deg2rad(dbuf[10+numStages+69]);
