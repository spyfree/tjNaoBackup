  ...
  */
  ...
  // void write(Out& stream) const {stream.write(points, sizeof(points));}
    ...
    */
	...
	void read(ifstream& tab)
	{
		for (int row=0;row<ySize;row++)
			for(int column=0;column<xSize;column++)
		{
			short x,y;
			tab.read((char*)(&x),sizeof(short));
			tab.read((char*)(&y),sizeof(short));
			points[row][column][0]=x;
			points[row][column][1]=y;
		}
	}
...
public:
  ...
  */
  const Vector2<int> getClosestCorner(const Vector2<int>& point, LinesPercept::Intersection::IntersectionType type, double dir) const;
