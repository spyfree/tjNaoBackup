...
public:
  ...
  */
    ...
    const std::vector<int>& selectedIndices, std::vector<double>& weightings)
  {
    ...
    const short distanceObserved = static_cast<short>(theLinesPercept.circleCenter.pos.abs());
    const double angleObserved = theLinesPercept.circleCenter.pos.angle();
