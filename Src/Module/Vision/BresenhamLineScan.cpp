/**
* @file Modules/ImageProcessor/ImageProcessorTools/BresenhamLineScan.cpp
* 
...
BresenhamLineScan::BresenhamLineScan(const Vector2<int>& start, const Vector2<double>& direction, const int imagewidth, const int imageheight)
{
	// Create a line through the point start with the given direction
	Geometry::GTLine line(Vector2<double>(static_cast<double>(start.x), static_cast<double>(start.y)),
					    direction);
	// Intersect line with image border
	Vector2<int> intersection1, intersection2;
	Geometry::getIntersectionPointsOfLineAndRectangle(Vector2<int>(0,0), Vector2<int>(imagewidth-1,imageheight-1),
...
BresenhamLineScan::BresenhamLineScan(const Vector2<int>& start, const double& direction,  const int imagewidth, const int imageheight)
{
	const Vector2<int> frameUpperLeft(0,0);
	const Vector2<int> frameLowerRight(imagewidth-1, imageheight-1);
	Geometry::GTLine scanLine(start, Vector2<double>(cos(direction), sin(direction)));
