...
bool GraphicsManager::beginAVIRecording(std::string& name)
{
	pAVIHelper = new AVIHelp(name.c_str(), 50);
	GLHelper::getGLH()->glPrintText(TextObject::COMMON_HINTS, TextObject::WHITE, 5000, "%s \"%s\".", "Begin Recording AVI", name.c_str());
	if(pAVIHelper)
		return true;
	else
		return false;
}
bool GraphicsManager::endAVIRecording()
{
	if(pAVIHelper){
		pAVIHelper->finish();
		delete pAVIHelper;
...
bool GraphicsManager::addAVIFrame(unsigned int width, unsigned int height)
{
	bool isOK = true;
	if(!pAVIHelper)
		return false;
	...
	//read in the screen data
	glReadPixels(0, 0, width, height, GL_RGBA, GL_UNSIGNED_BYTE, data);
	//add one frame to avi
	if(!(pAVIHelper->addFrame(data, width, height, 32)))
	{
		pAVIHelper->finish();
		delete pAVIHelper;
		pAVIHelper = 0;
...
bool GraphicsManager::getMultisampledFBOsState()
{
  return shader->getMultisampledFBOsState();
}

bool GraphicsManager::checkMultisampledFBOCapability()
{
