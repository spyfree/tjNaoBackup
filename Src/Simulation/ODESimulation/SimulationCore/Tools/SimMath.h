...
//       else
//         return 0;
		double h = sqrt(col[1].v[1] * col[1].v[1] + col[1].v[0] * col[1].v[0]);
		return atan2(col[1].v[2], sign(col[1].v[1]) * h);
    }
...
//       return (h ? -acos(h) : -M_PI) * (col[0].v[2] < 0 ? -1 : 1);
		double h = sqrt(col[0].v[0] * col[0].v[0] + col[0].v[1] * col[0].v[1]);
		return -atan2(col[0].v[2], sign(col[0].v[0]) * h);
    }
    ...
    double getZAngle() const
    {
       ...
       if(h)
       {
         ...
         return 0;
		//double h = sqrt(col[1].v[0] * col[1].v[0] + col[0].v[2] * col[0].v[2]);
		//return atan2(col[0].v[1], sign(col[1].v[0]) * h);
    }
