                      ...
                      const int fbo_reg)
{
  ...
  if(objectTree && visParams.visualizeSensors)
    precomputeSensorValues();
...
bool Simulation::getLightShowCones()
{
  return graphicsManager->getLightShowCones();
}
...
{
  std::vector<Sensorport*>::iterator sensor;
  double* dummy;
  double dummy2;
  for(sensor = sensorportList.begin(); sensor != sensorportList.end(); ++sensor)
  {
    if((*sensor)->getType() == doubleArraySensor)
    {
      (*sensor)->getValue(dummy);
    }
    else if((*sensor)->getType() == doubleSensor)
    {
      (*sensor)->getValue(dummy2);
    }
  }
}
                                                  ...
                                                  SimObject* simObject, bool selectButtons)
{
  ...
  if(result)
  {
    ...
    if(result)
    {
      ...
      selection.object = this->findRootOfMovableObject(selection.object);
      PhysicalObject* physObj = selection.object->castToPhysicalObject();
      if(physObj)
      {
        if(physObj->isGeometryTransformed())
        {
          if(this->compoundObjects.find(physObj->getMovableID()) != this->compoundObjects.end())
            this->compoundObjects[physObj->getMovableID()]->invertColors();
        }
        else
        {
...
bool Simulation::loadFile(const std::string& filename, const std::string& schemaName)
{
  ...
  {
	//printf errors
	  DOUT(("Loading ROS Files Error: \n"));
	 std::vector<ErrorDescription> errors;
	 errorManager.getAllErrors(errors);
	 for(int i = 0; i < errors.size(); ++i)
	 {
		 DOUT(("%d.\t%s: %s\n",i, errors[i].title.c_str(), errors[i].text.c_str()));
		 //DOUT(("%d.\t%s: %s, line(%d), column(%d)\n",i, errors[i].title.c_str(), errors[i].text.c_str(), errors[i].line, errors[i].column));
	 }
	//==============
...
int Simulation::getActuatorportIdByPartOfName(const std::vector<std::string>& parts) const
{
  int result = -1;
  for(unsigned int i=0; i<actuatorportList.size(); i++)
  {
...
int Simulation::getSensorportIdByPartOfName(const std::vector<std::string>& parts) const
{
  int result = -1;
  for(unsigned int i=0; i<sensorportList.size(); i++)
  {
    const std::string& currentName(sensorportList[i]->getName());
    std::string::size_type index = 0;
    for(unsigned j = 0; j < parts.size(); ++j)
    {
      index = currentName.find(parts[j], index);
...
Material* Simulation::getMaterial(const std::string& name) const
{
...
void Simulation::collisionCallback(dGeomID geom1, dGeomID geom2)
{
  ...
  else
  {
    ...
    if(n > 0)
    {
	  ...
	  //check FSRSensor collsion 
	  bool setFeedBack = false;
	  dJointFeedback* jFeedback = 0;
	  // collision sensors
	  if(!collisionSensors.empty())
	  {
		  ...
		  if(collisionSensors.find(bumperGeom) != collisionSensors.end())
		  {
			  ...
			  }else if(collisionSensors[bumperGeom]->getKind() == "FSRSensor")
			  {
				FSRSensor* fsr = collisionSensors[bumperGeom]->castToFSRSensor();
				if(fsr)
				{
					setFeedBack = true;
					jFeedback = fsr->getJointFeedBackBuf();
					fsr->setNumFeedbacks(n);
				}
			  }
		  ...
		  if(collisionSensors.find(bumperGeom) != collisionSensors.end())
		  {
			  if(this->collisionSensors[bumperGeom]->getKind() == "bumper")
			  {
				  Bumper* bumper = collisionSensors[bumperGeom]->castToBumper();
				  if(bumper)
				  {
      ...
      for(i=0; i<n; i++)
      {
		...
		if(setFeedBack && i < FSRSensor::MAX_FEEDBACKS){
			dJointSetFeedback(c, &jFeedback[i]);
		}
      }
...
void Simulation::postprocessObjectTree(SimObject* element, const std::string& path)
{
  ...
  {
    createNewMovableID();
    element->setMovableID(this->getCurrentMovableID());
  }
...
void Simulation::createCompoundPhysicalObjects(SimObject* object)
{
  if(object)
  {
    ...
    for(pos = childNodes->begin(); pos != childNodes->end(); ++pos)
    {
      ...
      if(movID != 0 && (*pos)->castToPhysicalObject() != NULL)
      {
        //movable objects
        std::map<int, CompoundPhysicalObject*>::const_iterator compoundObjIter;
        ...
        {
          this->compoundObjects[movID]->addObject(*pos);
        }
        else
        {
          ...
          this->compoundObjects[movID] = compoundObj;
        }
      }

      this->createCompoundPhysicalObjects(*pos);
    }
  }
...
void Simulation::collectCollisionSensors()
{
  ...
  {
    SimSensor* sensor = (*pos)->getSensor();
    if(sensor->getKind() == "bumper" || sensor->getKind() == "FSRSensor" || sensor->getKind() == "interactiveButton")
    {
...
void Simulation::collectCollisionSensorGeometry(SimSensor* sensor, SimObject* object)
{
  ObjectList::const_iterator pos;
  ObjectList* childNodes = object->getPointerToChildNodes();
  for(pos = childNodes->begin(); pos != childNodes->end(); ++pos)
  {
    if(object->castToJoint() == NULL && object->getMovableID() == (*pos)->getMovableID())
    {
...
{
  std::map<dGeomID, SimSensor*>::const_iterator iter;
  for(iter = collisionSensors.begin(); iter != collisionSensors.end(); ++iter)
  {
	...
	FSRSensor* fsr = iter->second->castToFSRSensor();
	if(fsr)
	{
		fsr->resetJointFeedback();
		fsr->setValue(0);
	}
  }
}
...
SimObject* Simulation::findRootOfMovableObject(SimObject* object) const
{
  ...
  if(parent->getMovableID() > 0 && (parent->getMovableID() == object->getMovableID() || parent->castToJoint() != NULL))
  {
...
void Simulation::registerControllerDrawing(const std::string& objectName, Controller3DDrawing* drawing)
{
  SimObject* object = getObjectReference(objectName);
  object->attachDrawing(drawing);
}

void Simulation::unregisterControllerDrawing(const std::string& objectName, Controller3DDrawing* drawing)
{
  SimObject* object = getObjectReference(objectName);
  object->removeDrawing(drawing);
}
...
bool Simulation::getMultisampledFBOsCapability()
{
  if(graphicsManager)
  {
    return (graphicsManager->checkMultisampledFBOCapability());
  }
  return false;
}

bool Simulation::getMultisampledFBOsState()
{
  if(graphicsManager)
    return (graphicsManager->getMultisampledFBOsState());
  return false;
}
...
void Simulation::setObjectLinearVelocity(SimObject* object, double vx, double vy, double vz)
{
	if(object)
	{
		if(object != objectTree)
		{
			...
			if(physObj)
			{
				...
				if(body)
				{
					dBodySetLinearVel(*body,dReal(vx), dReal(vy), dReal(vz));
					//dBodySetAngularVel(*body, 0,0,0);
				}
			}
		}
		...
		for(pos = childNodes->begin(); pos != childNodes->end(); ++pos)
		{
			setObjectLinearVelocity(*pos, vx, vy, vz);
		}
	}
}

void Simulation::setObjectAngularVelocity(SimObject* object, double vx, double vy, double vz)
{
	if(object)
	{
		if(object != objectTree)
		{
			...
			if(physObj)
			{
				...
				{
					//dBodySetLinearVel(*body,dReal(vx), dReal(vy), dReal(vz));
					dBodySetAngularVel(*body, dReal(vx), dReal(vy), dReal(vz));
				}
			}
		}
