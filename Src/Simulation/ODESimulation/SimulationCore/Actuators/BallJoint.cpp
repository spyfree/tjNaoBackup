                                       ...
                                       ErrorManager* errorManager)
{
  ...
  else if(name == "UserDefinedMotor")
  {
    parseMotor(name, attributes);
  }
  else if(name == "EulerMotor")
  {
    parseMotor(name, attributes);
  }
...
{
  this->currentMotor->setValue(value, port);
}
...
void BallJoint::computeValue(double& value, int portId)
{
  if(portId == MOTOR_AXIS_1)
    value = (dJointGetAMotorParam(*(this->currentMotor->getPhysicalMotor()), dParamVel));
  else if(portId == MOTOR_AXIS_2)
    value = (dJointGetAMotorParam(*(this->currentMotor->getPhysicalMotor()), dParamVel2));
  else if(portId == MOTOR_AXIS_3)
    value = (dJointGetAMotorParam(*(this->currentMotor->getPhysicalMotor()), dParamVel3));
}
