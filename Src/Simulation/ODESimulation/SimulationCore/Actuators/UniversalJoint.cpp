                       ...
                       std::vector<Actuator*>& actuatorList) 
{
  if(axis1 != 0 && axis2 != 0)
  {
    Sensorport* valueSensor = new Sensorport
      ("jointAxis1SensorValue", AXIS1_JOINT, doubleSensor, this, axis1->minValue, axis1->maxValue);
    sensorportList.push_back(valueSensor);
    valueSensor = new Sensorport
      ("jointAxis2SensorValue", AXIS2_JOINT, doubleSensor, this, axis2->minValue, axis2->maxValue);
    sensorportList.push_back(valueSensor);
...
void UniversalJoint::createMotorPhysics()
{
  // ONLY TEMPORARY DISABLED
/*
  if(this->axis1->motor != 0 || this->axis2->motor != 0)
  {
    ...
    {
      this->axis1->motor->setPhysicalMotor(physMotor);
      numOfMotors++;
      ...
      if(b1 != NULL)
        dJointSetAMotorAxis(physMotor, 0, 1, dReal(this->axis1->direction.v[0]), dReal(this->axis1->direction.v[1]), dReal(this->axis1->direction.v[2]));
      else
        dJointSetAMotorAxis(physMotor, 0, 0, dReal(this->axis1->direction.v[0]), dReal(this->axis1->direction.v[1]), dReal(this->axis1->direction.v[2]));

      dJointSetAMotorParam(physMotor, dParamFMax, dReal(this->axis1->motor->getMaxForce()));

      if(this->axis1->motor->getMotorType() == ANGULAR)
      {
        dJointSetAMotorParam(physMotor, dParamLoStop, dReal(Functions::toRad(this->axis1->minValue)));
        dJointSetAMotorParam(physMotor, dParamHiStop, dReal(Functions::toRad(this->axis1->maxValue)));
        // this has to be done due to the way ODE sets joint stops
        dJointSetAMotorParam(physMotor, dParamLoStop, dReal(Functions::toRad(this->axis1->minValue)));
      }
    }
    ...
    {
      this->axis2->motor->setPhysicalMotor(physMotor);
      numOfMotors++;
      dJointAttach(physMotor, dJointGetBody(this->physicalJoint, 0), dJointGetBody(this->physicalJoint, 1));

      dBodyID b1= dJointGetBody(this->physicalJoint, 0);
      if(b1 != NULL)
        dJointSetAMotorAxis(physMotor, 1, 1, dReal(this->axis2->direction.v[0]), dReal(this->axis2->direction.v[1]), dReal(this->axis2->direction.v[2]));
      else
        dJointSetAMotorAxis(physMotor, 1, 0, dReal(this->axis2->direction.v[0]), dReal(this->axis2->direction.v[1]), dReal(this->axis2->direction.v[2]));

      dJointSetAMotorParam(physMotor, dParamFMax2, dReal(this->axis2->motor->getMaxForce()));

      if(this->axis2->motor->getMotorType() == ANGULAR)
      {
        dJointSetAMotorParam(physMotor, dParamLoStop2, dReal(Functions::toRad(this->axis2->minValue)));
        dJointSetAMotorParam(physMotor, dParamHiStop2, dReal(Functions::toRad(this->axis2->maxValue)));
        // this has to be done due to the way ODE sets joint stops
        dJointSetAMotorParam(physMotor, dParamLoStop2, dReal(Functions::toRad(this->axis2->minValue)));
      }
    }

    dJointSetAMotorNumAxes(physMotor, numOfMotors);
  }
*/
}
