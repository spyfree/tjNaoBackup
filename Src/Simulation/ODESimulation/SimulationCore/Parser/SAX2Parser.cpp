...
#include <PhysicalObjects/CappedCylinder.h>
...
SAX2Parser::SAX2Parser():parsingState(PARSING_SIMULATION), coefficientTablesGenerated(false)
{
  ...
  majorElementNames.insert("CappedCylinder");
  ...
  majorElementNames.insert("SimpleCappedCylinder");
  ...
  majorElementNames.insert("FSRSensor");
...
void SAX2Parser::endDocument()
{
  ...
  simulation->postprocessObjectTree(currentNode,path);
}
                              ...
                              int line, int column)
{
  ...
  else if(isMajorElement(name))
  {
    ...
    else if(name == "CappedCylinder" || name == "SimpleCappedCylinder")
    {
      CappedCylinder* cappedCylinder = new CappedCylinder(attributeSet);
      currentNode->addChildNode(cappedCylinder, true);
      currentNode = cappedCylinder;
    }
    ...
    else if(name == "Led")
    {
      ...
      led->setPosition(currentNode->getPosition());
	...
	else if(name == "FSRSensor")
	{
		FSRSensor* fsr = new FSRSensor(attributeSet);
		currentNode->addChildNode(fsr, true);
		currentNode = fsr;
	}
  ...
  else if(name == "Scene")
  {
    ...
    for(unsigned int i=0; i<attributeSet.size(); i++)
    {
      std::string attributeName(attributeSet[i].attribute);
      if(attributeName == "name")
      {
        this->rootNode->setName(attributeSet[i].value);
      }
    }
  }
